 ...
 *    G                  → Cuadricula
 *    A / a              → Cerrar / abrir TODOS los dedos
 *    R                  → Restablecer posicion
 *    ESC                → Salir
 *
 *  Articulaciones individuales (teclado):
 *    Pulgar  MCP: Q/q   IP:  W/w
 *    Indice  MCP: E/e   PIP: R/r   DIP: T/t
 *    Medio   MCP: Y/y   PIP: U/u   DIP: I/i
 *    Anular  MCP: O/o   PIP: P/p   DIP: [/]
 *    Menique MCP: F/f   PIP: G/g   DIP: H/h
 * =====================================================================
 */
...
static void drawHUD()
{
    ...
    glBegin(GL_QUADS);
      glVertex2f(12,8); glVertex2f(415,8);
      glVertex2f(415,410); glVertex2f(12,410);
    glEnd();
    glDisable(GL_BLEND);
    glColor3f(0.28f,0.50f,0.82f);
    glBegin(GL_LINE_LOOP);
    ...
    glColor3f(0.72f,0.88f,1.0f);
    drawText(18, 392, "CONTROLES", GLUT_BITMAP_HELVETICA_12);
    ...
    static const CtrlRow ctrl[] = {
        ...
        {"--- GLOBALES -------------------","",true},
        {"A",                "Cerrar todos los dedos",false},
        {"a",                "Abrir  todos los dedos",false},
        {"R",                "Restablecer posicion",false},
        {"ESC",              "Salir",false},
        {"--- PULGAR (naranja) -----------","",true},
        {"Q / q",            "MCP  doblar / extender",false},
        {"W / w",            "IP   doblar / extender",false},
        {"--- INDICE (verde) -------------","",true},
        {"E / e",            "MCP  doblar / extender",false},
        {"R / r",            "PIP  doblar / extender",false},  // nota: R global tambien resetea
        {"T / t",            "DIP  doblar / extender",false},
        {"--- MEDIO (azul) ---------------","",true},
        {"Y / y",            "MCP  doblar / extender",false},
        {"U / u",            "PIP  doblar / extender",false},
        {"I / i",            "DIP  doblar / extender",false},
        {"--- ANULAR (lila) --------------","",true},
        {"O / o",            "MCP  doblar / extender",false},
        {"P / p",            "PIP  doblar / extender",false},
        {"[ / ]",            "DIP  doblar / extender",false},
        {"--- MENIQUE (rojo) -------------","",true},
        {"F / f",            "MCP  doblar / extender",false},
        {"H / h (Shift/no)", "PIP  doblar / extender",false},
        {"N / n",            "DIP  doblar / extender",false},
    };
    int cy2=374;
    for(auto& cr2:ctrl){
        if(cr2.header){
    ...
    for(int i=0;i<5;i++){
        ...
        // Fondo del dedo activo (todos siempre visibles)
        glColor4f(dcr[i]*0.3f, dcg[i]*0.3f, dcb[i]*0.3f, 0.6f);
        glEnable(GL_BLEND);
        glBegin(GL_QUADS);
...
static void keyboard(unsigned char key, int, int)
{
    ...
    // Global: cerrar / abrir / reset
    case 'A': // cerrar todos
        for(int i=0;i<5;i++) for(int j=0;j<3;j++) bend(i,j,+STEP*3); break;
    case 'a': // abrir todos
        for(int i=0;i<5;i++) for(int j=0;j<3;j++) bend(i,j,-STEP*3); break;
    case 'R':
    ...
    // PULGAR (naranja)
    case 'Q': bend(0,0,+STEP); break;
    case 'q': bend(0,0,-STEP); break;
    case 'W': bend(0,1,+STEP); break;
    case 'w': bend(0,1,-STEP); break;

    // INDICE (verde)
    case 'E': bend(1,0,+STEP); break;
    case 'e': bend(1,0,-STEP); break;
    // 'R' ya es reset global; usamos Shift+R para PIP indice no colisiona
    // Usamos teclas alternativas para evitar conflicto con R=reset
    case 'r': bend(1,1,-STEP); break;   // indice PIP extender
    // Indice PIP doblar: usamos Ctrl no disponible, reasignamos a 'S'
    case 'S': bend(1,1,+STEP); break;   // Indice PIP doblar
    case 's': bend(1,2,-STEP); break;   // Indice DIP extender
    case 'T': bend(1,2,+STEP); break;
    case 't': bend(1,2,-STEP); break;

    // MEDIO (azul)
    case 'Y': bend(2,0,+STEP); break;
    case 'y': bend(2,0,-STEP); break;
    case 'U': bend(2,1,+STEP); break;
    case 'u': bend(2,1,-STEP); break;
    case 'I': bend(2,2,+STEP); break;
    case 'i': bend(2,2,-STEP); break;

    // ANULAR (lila)
    case 'O': bend(3,0,+STEP); break;
    case 'o': bend(3,0,-STEP); break;
    case 'P': bend(3,1,+STEP); break;
    case 'p': bend(3,1,-STEP); break;
    case '[': bend(3,2,+STEP); break;
    case ']': bend(3,2,-STEP); break;

    // MENIQUE (rojo)
    case 'F': bend(4,0,+STEP); break;
    case 'f': bend(4,0,-STEP); break;
    case 'H': bend(4,1,+STEP); break;
    case 'h': bend(4,1,-STEP); break;
    case 'N': bend(4,2,+STEP); break;
    case 'n': bend(4,2,-STEP); break;
    }
    glutPostRedisplay();
}
