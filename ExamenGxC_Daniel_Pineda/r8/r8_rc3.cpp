...
static void drawNail(float radius, float length)
{
    setNailMaterial();
    glPushMatrix();
      // Cerca de la punta (no en la base/articulacion) y sobre la superficie frontal
      glTranslatef(0, length*0.62f, radius*0.92f);
      glScalef(radius*0.78f, length*0.30f, radius*0.40f);
...
static void drawHand()
{
    ...
    // ── PULGAR ─────────────────────────────────────────────────────────────
    glPushMatrix();
      // Mas arriba (a la altura media de la palma, no en la base/muneca) y
      // con una inclinacion mas vertical para que se vea extendido hacia
      // arriba como los demas dedos, solo desviado hacia el costado.
      glTranslatef(-0.58f, -0.02f, 0.16f);
      glRotatef(-40, 0,0,1);   // inclinacion lateral (mas vertical que antes)
