...
static void drawHand()
{
    ...
    // ── PULGAR ─────────────────────────────────────────────────────────────
    glPushMatrix();
      // Posicion base del pulgar (lateral izquierdo, separado de la palma)
      glTranslatef(-0.56f, -0.18f, 0.14f);
      glRotatef(-30, 0,1,0);   // abduccion: lo saca del plano de la palma hacia adelante
      glRotatef(-68, 0,0,1);   // apertura lateral (separado, no pegado)
      glRotatef(  5, 1,0,0);   // tilt minimo hacia adelante
      drawThumb();
    glPopMatrix();
