...
// Dibuja una uña plana cerca de la punta de la falange distal
// side=+1 → cara frontal (como estaba el pulgar, que ya quedaba bien)
// side=-1 → cara opuesta/dorso (la que necesitan los demas dedos)
static void drawNail(float radius, float length, float side=1.0f)
{
    setNailMaterial();
    glPushMatrix();
      // Cerca de la orilla/punta del dedo (no en la articulacion de atras)
      glTranslatef(0, length*0.74f, side*radius*0.92f);
      glScalef(radius*0.78f, length*0.34f, radius*0.40f);
...
static void drawPhalanx(float radius, float length, bool hasnail=false, float nailSide=1.0f)
{
    setSkinMaterial();
    ...
    glPopMatrix();
    if(hasnail) drawNail(radius, length, nailSide);
}
                       ...
                       float rDist, float lDist)
{
    ...
    drawPhalanx(rDist, lDist, true, -1.0f);  // unia en el dorso del dedo
}
...
static void drawThumb()
{
    ...
    drawPhalanx(rDist, lDist, true, 1.0f);  // unia del pulgar: lado original (ya estaba bien)
}
...
static void drawHand()
{
    ...
    // ── PULGAR ─────────────────────────────────────────────────────────────
    glPushMatrix();
      // Nace al costado de la palma y se abre HACIA AFUERA (lejos del centro
      // de la mano). El signo de la rotacion en Z es el que controla esto:
      // positivo = se abre hacia afuera; negativo lo cruzaba hacia el centro
      // de la palma, por eso se veia "pegado"/atravesando la palma.
      glTranslatef(-0.50f, -0.05f, 0.14f);
      glRotatef( 42, 0,0,1);   // abre el pulgar hacia afuera y hacia arriba
      glRotatef(  6, 1,0,0);   // tilt leve hacia adelante
      drawThumb();
    glPopMatrix();
