...
static void drawHand()
{
    ...
    // ── PULGAR ─────────────────────────────────────────────────────────────
    glPushMatrix();
      // Posicion base del pulgar: separado de la palma, MISMA convencion de
      // ejes que los demas dedos (solo rotacion en Z + tilt leve en X) para
      // que extender/contraer se sienta igual que en el resto de los dedos.
      glTranslatef(-0.60f, -0.16f, 0.10f);
      glRotatef(-62, 0,0,1);   // apertura lateral: lo separa claramente de la palma
      glRotatef(  8, 1,0,0);   // tilt leve hacia adelante
      drawThumb();
    glPopMatrix();
