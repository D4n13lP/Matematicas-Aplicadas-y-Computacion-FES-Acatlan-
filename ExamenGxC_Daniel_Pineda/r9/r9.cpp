...
static void drawHUD()
{
    ...
    // ── Panel definicion activa (dos lineas: etiqueta + definicion completa) ──
    const std::string& defActiva = usePuntual ? defPuntual : defGlobal;
    float panX = 14.0f;
    float panW = winW - 28.0f;
    // Dos filas de texto: fila superior = etiqueta, fila inferior = definicion
    float lineH   = 16.0f;
    float padV    = 6.0f;
    float panH    = lineH * 2 + padV * 2;
    float panY    = winH - 90.0f;            // borde superior del panel
    ...
    // Linea 1: etiqueta del modo activo
    ...
    // Linea 2: definicion completa sin truncar
