 ...
 *    g++ figuras3d.cpp -o figuras3d.exe -lfreeglut -lopengl32 -lglu32
 *  Compilar Linux:
 *    g++ figuras3d.cpp -o figuras3d -lGL -lGLU -lglut -lm
 *
 *  Controles:
 *    ESPACIO          → Siguiente figura
 *    B / V            → Figura anterior / siguiente
 *    Tab              → Cambiar modo: Solido / Alambre / Ambos
 *    Arrastrar raton  → Rotar figura
...
static void drawPiramideHex(bool wire)
{
    ...
    GLenum m = wire ? GL_LINE_LOOP : GL_TRIANGLES;
...
static void drawOctaedro(bool wire)
{
    ...
    int faces[8][3] = {
        ...
        {0,4,3},{4,1,3},{1,5,3},{5,0,3}  // corregido: las 4 inferiores
    };
    // Las 8 caras del octaedro regular
    int f8[8][3] = {
        {0,2,4},{2,1,4},{1,3,4},{3,0,4},
...
// ═════════════════════════════════════════════════════════════════════════════
//  DODECAEDRO  (12 pentagonos) — vertices y winding verificados
// ═════════════════════════════════════════════════════════════════════════════
static void drawDodecaedro(bool wire)
{
    const float phi = (1.0f + sqrtf(5.0f)) * 0.5f;   // ~1.618
    const float sc  = 0.92f;
    const float p   = sc * phi;
    const float ip  = sc / phi;
    const float u   = sc * 1.0f;

    // 20 vertices (3 clases de coordenadas ciclicas)
    float vv[20][3] = {
        // (±u, ±u, ±u)
        { u, u, u},  // 0
        { u, u,-u},  // 1
    ...
    // 12 pentagonos — winding CCW visto desde el exterior
    int fi[12][5] = {
        { 0, 8, 4,14,12},  // top-front
        { 0,12, 1,17,16},  // right-top
        { 0,16, 2,10, 8},  // front-right
        { 4, 8,10, 6,18},  // front-left
        { 4,18,19, 5,14},  // left-top
        { 1,12,14, 5, 9},  // back-top
        { 2,16,17, 3,13},  // right-bottom
        { 2,13,15, 6,10},  // front-bottom
        { 6,15, 7,19,18},  // left-bottom
        { 3,17, 1, 9,11},  // back-right
        { 5,19, 7,11, 9},  // back-left
        { 3,11, 7,15,13},  // bottom-back
    };

    for (int i = 0; i < 12; i++) {
        // Normal: centroide de la cara (siempre apunta afuera en solido centrosimetrico)
...
{
    const float R  = 0.80f; // radio mayor
    const float r  = 0.35f; // radio menor
    const int   NS = 48;    // segmentos del tubo
    const int   NT = 64;    // segmentos del anillo
    ...
    for (int i = 0; i < NT; i++) {
        ...
        for (int j = 0; j <= NS; j++) {
            ...
            for (int k = 0; k < 2; k++) {
                ...
                float y = r*sv;
                float z = (R + r*cv)*su;
                // normal (vector desde centro del tubo)
                float nx = cv*cu;
                float ny = sv;
                float nz = cv*su;
                glNormal3f(nx, ny, nz);
...
static void drawHUD()
{
    ...
    drawText(14, winH - 34,
             "FIGURAS 3D  —  ALAMBRE Y SOLIDO",
             GLUT_BITMAP_HELVETICA_18);

    // Nombre de la figura actual
    glColor3f(0.45f, 0.82f, 1.0f);
    char titulo[80];
    snprintf(titulo, sizeof(titulo), "Figura actual:  %s", figNames[currentFig]);
    drawText(16, winH - 58, titulo, GLUT_BITMAP_HELVETICA_12);

    // Modo actual (junto al titulo)
    ...
    glBegin(GL_QUADS);
      glVertex2f(12,  8); glVertex2f(335, 8);
      glVertex2f(335,215); glVertex2f(12,215);
    glEnd();
    glDisable(GL_BLEND);
    ...
    drawText(18, 197, "CONTROLES", GLUT_BITMAP_HELVETICA_12);
    ...
    for (auto& r : rows) {
        char buf[80];
        snprintf(buf, sizeof(buf), "%-26s%s", r.k, r.d);
...
static void display()
{
    ...
    if (renderMode == RM_SOLID || renderMode == RM_BOTH) {
        ...
        // Offset para que el alambre superpuesto no z-fight
    ...
    if (renderMode == RM_WIRE || renderMode == RM_BOTH) {
        ...
        if (renderMode == RM_BOTH) {
            ...
            // Alambre puro: color del material aproximado
...
static void keyboard(unsigned char key, int, int)
{
    switch (key) {
        ...
        case 'r': case 'R': rotX=25; rotY=-35; zoom=1.0f; break;
        case 'g': case 'G': showGrid=!showGrid; break;
        // Teclas 1-6 para ir directo a cada figura
...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow("Figuras 3D - Alambre y Solido");

    glClearColor(0.11f, 0.14f, 0.19f, 1.0f);

    // ── Iluminacion: 3 luces para resaltar vertices y filos ───────────────
    glEnable(GL_LIGHTING);
